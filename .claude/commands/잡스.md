...
1. `agent/planning_잡스_guide.txt` — 잡스 행동 강령 (Big Idea 발굴·채널 믹스·비전통적 채널·인플루언서 전략 기준)
2. `agent/research_스티브_guide.txt` — 스티브 행동 강령 (인풋 기준 이해 — Nova는 스티브 산출물을 인풋으로 받음)
...
작업 완료 시 `c:/Users/gnb_system/Desktop/Purit/purit-marketer-agent/memory/team_memory.txt`에 기록하고 `agent/planning_잡스_guide.txt` LEARNED 섹션을 업데이트할 것.
