-- 116_fix_badge_rpc_app_metadata.sql
-- 니체 보안 전수조사 1단계(#9) — check_and_award_badges 어드민 판별 D-58 수정.
--
--  배경: 077 의 호출자 검증이 raw_user_meta_data->>'role'='admin' 로 어드민을 판별(077:46).
--    user_metadata 는 supabase.auth.updateUser({data:{role:'admin'}}) 로 본인이 위조 가능(D-58) →
--    패널이 user_metadata.role 위조 시 어드민 분기를 통과해 타 패널(p_panel_id≠본인)에게 뱃지 부여 가능.
--    형제 honor RPC(add_panel_honor_points·apply_honor_decay)는 032 에서 app_metadata 로 고쳤으나,
--    077 이 021 을 덮어쓰며 이 함수만 user_metadata 어드민 분기가 잔존(098 도 미포함).
--    심각도 낮음(뱃지는 cosmetic·honor/정산 무관)이나 권한 판별 일관성(D-58) 차원에서 정비.
--
--  수정: 077 본문 100% 유지, 호출자 검증 블록(45~57)만 112의 is_caller_admin()(app_metadata 전용·
--    EXISTS·NULL 안전 D-134)로 교체. 본인 폴백(p_panel_id=본인 panel)은 그대로.
--    → 정상 경로 무영향(패널 본인 호출·어드민 client 호출 모두 통과), 클라이언트 무변경.
--
-- ⚠️ Supabase SQL Editor 수동 실행 필요. is_caller_admin() 헬퍼는 112 선행 적용 전제.
...
BEGIN
  -- ── 호출자 검증 (raw_app_meta_data 전용 — D-58/D-134, 112 is_caller_admin 헬퍼) ──
