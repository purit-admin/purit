...
const AXES = [
  ...
  { key: 'value',       label: '지각 가치',     icon: '◆', color: '#6366F1', desc: '가격 대비 가치가 납득되는가?' },
...
export default function PricingTest() {
  ...
  function resetForm() {
    ...
    setCreateStep(0);
    setMissionTitle(''); setPricingDesc(''); setPricingImage(null);
    ...
    try {
      ...
      if (co) {
        const { data: missionsData } = await supabase
          ...
          .eq('company_id', co.id).eq('type', 'pricing').eq('dismissed', false)
      ...
      console.error('[PricingTest load]', err);
      setLoading(false);
    }
  }
  ...
  const draftKey = companyId ? `purit_form_draft_pricing_${companyId}` : null;
  ...
  useEffect(() => {
    ...
    if (!(missionTitle || pricingDesc || productDescription || industry)) return;
    try {
      localStorage.setItem(draftKey, JSON.stringify({
        missionTitle, pricingDesc, pricingImage, productDescription, industry,
        panelSize, selectedQuestions, localCustomQs, careerLevels, createStep, missionUuid, savedAt: Date.now(),
      }));
    } catch {}
  }, [draftKey, draftId, view, missionTitle, pricingDesc, pricingImage, productDescription, industry,
  ...
  function restoreLocalDraft() {
    ...
    if (d.pricingDesc != null) setPricingDesc(d.pricingDesc);
    if (d.pricingImage !== undefined) setPricingImage(d.pricingImage);
  ...
  async function saveDraft() {
    ...
    try {
      ...
      console.error('[PricingTest] 임시 저장 실패:', e.message);
  ...
  function openDraftOrActiveForEdit(missionId) {
    ...
    supabase.from('missions').select('*').eq('id', missionId).single().then(({ data: ms }) => {
      ...
      if (parsed.panelSize) setPanelSize(parsed.panelSize);
      setView('create');
    }).catch(e => console.error('[PricingTest] 이어쓰기 로드 실패:', e.message));
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { error: tErr } = await supabase.from('pricing_tests').insert({
        company_id: companyId, status: 'active', mission_id: targetId, template_id: null,
      });
  ...
  return (
    <div className="page-wrap" style={{ padding: '40px 48px', maxWidth: 900, animation: 'fadeUp 0.5s ease both' }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontSize: 12, fontFamily: 'var(--font-sans)', color: 'var(--text-2)', marginBottom: 8, letterSpacing: '0.1em' }}>PRICING TEST</div>
        ...
        <div>
            ...
            {createStep === 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  />
                  <div>
                    <div style={{ fontSize: 12, fontFamily: 'var(--font-sans)', color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>이미지 업로드 (선택)</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 20 }}>
            ...
            {createStep < STEPS.length - 1 ? (
              ...
              <Btn onClick={() => setShowSubmitConfirm(true)} disabled={teamRole === 'viewer' || submitting || (creditsChargedOnSubmit && creditBalance != null && calcCredits(panelSize, careerLevels, 'sub') > creditBalance)}>
        ...
        {restorable && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', padding: '14px 18px', marginBottom: 16, background: 'rgba(16,54,125,0.06)', border: '1px solid rgba(16,54,125,0.25)', borderRadius: 10 }}>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text)' }}>작성 중이던 가격 검증이 있습니다</div>
                ...
                <Card key={m.id} style={{ cursor: 'pointer', border: isDraft ? '1px dashed #f59e0b' : undefined }}
                  ...
                  <div className="mc-row">
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{m.title || '가격 페이지 검증'}</div>
