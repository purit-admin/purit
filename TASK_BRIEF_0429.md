...
드래그를 하면 그제서야 명확성등 5개의 평가가 나오는 것이 아닌 5개가 이미 나눠져 있고, 명확성을 클릭하면 거기서 드래그가 자동으로 명확성임을 알고 거기서 평가만 하는 방식이었으면 좋겠다. 다 제출하면 총평을 적는 것도 따로 나오게 해라.
그리고 기업이 피드백 결과에서 피드백 상세를 볼 떄도 5개의 차원을 하나하나 이런 방식으로 볼 수 있게 하고, 마지막 총평을 볼 떄 원래 있던 거인 점수들 한 번에 보는 것과 총평이 같이 나왔으면 좋겠다. 
