 ...
 * - 5대 지표 이미지 어노테이션 (좌표+점수+코멘트, 다중 이미지 지원)
...
const COMMENTS = {
  ...
  differentiation: {
    high: [
      ...
      '정량적 5대 지표 평가 프레임이 독자적인 방법론으로 분명히 부각됩니다.',
...
const OVERALL_HIGH = [
  '전반적으로 CRO SaaS의 핵심 가치를 잘 전달하고 있습니다. "실제 타겟 패널"이라는 차별화 포인트가 인상적이며, 5대 지표 평가 프레임이 독창적입니다. 다만 히어로 섹션의 카피를 고객 언어로 좀 더 바꾸고 상단에 구체적인 전환율 개선 수치를 배치한다면 즉각적인 관심 유도에 효과적일 것입니다.',
  ...
  '5대 지표 평가 방법론이 페이지 전반에 일관되게 표현되어 있고, 가격 플랜 비교 테이블이 명확해 의사결정을 돕습니다. 무료 체험이나 파일럿 제안 섹션이 추가된다면 도입 장벽을 낮출 수 있을 것입니다. 전환을 가로막는 마지막 불안 요소(패널 품질, 결과 신뢰도)에 대한 답변이 있으면 좋겠습니다.',
  ...
  '전반적으로 깔끔하고 전문적인 디자인입니다. 특히 5대 지표 레이더 차트 미리보기가 서비스의 정량적 특성을 직관적으로 전달합니다. 플랜 간 차이가 명확하게 표현되어 있어 구독 플랜 선택이 어렵지 않습니다. 실 고객 인터뷰 영상이나 짧은 사용 데모가 추가되면 전환율이 더 올라갈 것입니다.',
  '타겟 고객의 언어를 잘 사용하고 있으며 각 섹션 CTA 문구가 명확합니다. 특히 NDA 보장 + 5대 지표 평가 조합이 기존 설문 툴과의 차별점을 인상적으로 만들어냅니다. 도입 기업 구체 사례(업종, 개선 수치)와 30일 무료 체험 제안이 추가된다면 망설이는 고객의 최종 전환에 결정적인 역할을 할 것입니다.',
];
...
// 퓨릿 점수 품질 계층 (OVERALL_* 선택 + 지표 점수 범위 제어)
...
const TEXT_ANSWERS = [
  ...
  '5대 지표 평가 프레임이 우리 서비스에 어떻게 맞춤 적용되는지 구체적인 설명이 없습니다.',
...
// 품질 계층에 따라 지표 점수 범위 제어 (low=2~3, mid=2~4 혼합, high=3~5)
